...
public class ScreenImage {
     ...
     *  @return	image the image for the given region
     */
    public static BufferedImage createImage(final JComponent component, final Rectangle region) {
     ...
     */
    public static BufferedImage createImage(final Component component)
            throws AWTException {
     ...
     * @return image the image for the given region
     */
    public static BufferedImage createImage(final Rectangle region)
            throws AWTException {
        final BufferedImage image = new Robot().createScreenCapture(region);
        return image;
    }
    ...
    public static void writeImage(final BufferedImage image, final String fileName)
        ...
        if (offset == -1) {
            final String message = "file suffix was not specified";
            throw new IOException(message);
        }

        final String type = fileName.substring(offset + 1);

        if (types.contains(type)) {
            ImageIO.write(image, type, new File(fileName));
        } else {
            final String message = "unknown writer file suffix (" + type + ")";
            throw new IOException(message);
        }
    }

    static void layoutComponent(final Component component) {
